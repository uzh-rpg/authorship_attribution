...
class CustomDistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.ref_embedding = nn.Linear(self.ref_embedding_size, self.ref_concat_size)
        self.pre_classifier = nn.Linear(self.ref_concat_size, config.dim)
        self.classifier = nn.Linear(config.dim, config.num_labels)
        self.dropout = nn.Dropout(config.seq_classif_dropout)

        self.lstm = torch.nn.LSTM(self.ref_embedding_size, self.ref_embedding_size, 2, batch_first=True)
